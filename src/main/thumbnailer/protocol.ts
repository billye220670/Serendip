...
async function serveFullImage(filePath: string): Promise<Response> {
  ...
  if (ext === '.heic' || ext === '.heif') {
    ...
    return new Response(new Uint8Array(buf), { headers: { 'Content-Type': 'image/jpeg', 'Cache-Control': 'max-age=3600' } })
  }

  const mime = IMAGE_MIME[ext] ?? 'application/octet-stream'
  const buf = await readFile(filePath)
  return new Response(new Uint8Array(buf), { headers: { 'Content-Type': mime, 'Cache-Control': 'max-age=3600' } })
}
