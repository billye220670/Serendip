...
- `categories/index.ts` — collection CRUD + item membership. `listCategories` returns each category's `position` (drag-reorder order) and a subquery `itemCount`. `reorderCategories(orderedIds)` rewrites `position` in a transaction. `addItemsToCategory` uses `INSERT OR IGNORE` (dedupes; returns count actually added) — takes an array, so stage 5 batch-add reuses it as-is. `removeItemsFromCategory(categoryId, fileIds)` is the transactional batch counterpart to the single-item remove (added in stage 5). Deleting a category relies on the schema's `ON DELETE CASCADE` to clear `category_items`. Unique-name violations are rethrown as friendly Chinese `Error` messages.
...
- `App.tsx` — sidebar + header shell. Routes the main pane between `EmptyState` / `ScanProgressPanel` / `ExploreView` / `CategoryView` based on `useLibraryStore`'s `view` (`{kind:'explore'} | {kind:'category', id}`). **Hosts the single app-wide `DndContext`** so drags cross from the main pane into the sidebar. Owns the category create/rename/delete dialogs (`PromptDialog` / `ConfirmDialog`), the `DragOverlay`, and the move/copy drop menu. The header's right edge holds the `gridSize` toggle (小/中/大); header + toolbar use the `bg-glass` frosted tint.
- `stores/ui.ts` — Zustand with `persist` middleware (`localStorage` key `serendip-ui`). Holds `theme` + `exploreMode` + `gridSize` (`small`/`medium`/`large`, cycled by `cycleGridSize` — the masonry thumbnail-size toggle in the header); on rehydrate it writes `data-theme` to `documentElement` so Tailwind theme variables in `assets/main.css` apply before first paint.
- `stores/library.ts` — Zustand (not persisted). Holds `view` (current main-pane route) alongside scan state. `startScan` subscribes to `onScanProgress` (returns an unsubscribe), invokes `scanRoot`, then `loadStats` on completion. `categoryRefreshNonce` + `bumpCategoryRefresh()` let App force the **current** `CategoryView` to reload (used after a cross-category "移动到此" removes items from the category being viewed).
- `lib/grid.ts` — `getColumns(containerWidth, gridSize)` shared by every masonry view: derives column count from a per-size target column width (small≈190 / medium≈270 / large≈380px). Single source of truth so the header size toggle affects all grids identically.
- `stores/categories.ts` — Zustand (not persisted). Mirrors the `categories` table; all mutating actions call the IPC then refresh, except `reorder` / `addItems` / `removeItem` / `removeItems` which patch local state optimistically (reorder by index, itemCount by delta) to avoid flicker. Note `category_items` is many-to-many — a file can sit in multiple categories, which is what "复制到此" relies on.
- `stores/selection.ts` — Zustand (not persisted). Stage 5 multi-select: holds a `Set<number>` of selected fileIds, `active` (selection-mode flag), and `anchor` (for Shift-range). Each mutator returns a **fresh `Set`** so subscribers refresh. Cards subscribe to `selected.has(id)` / `active` individually (a primitive output, so only the toggled card re-renders). **Selection mode persists until the user exits** — `deselectAll` empties the set but stays in mode (toolbar's "清空" + every batch op use this), only `clear` leaves mode (toolbar ✕ / view unmount). Exports `useGridSelection(items)` — a hook the views call with their current ordered list; it owns the Shift-range math (anchor→target slice of `items`) and exposes `handleSelectClick` / `handleLongPress` / `handleSelectAll` / `getSelectedIds` / `deselectAll` / `clear` plus `selectedCount` / `selectionActive`.
- `views/Explore.tsx` — masonry layout via `react-photo-album` (columns via `getColumns(w, gridSize)`). Pulls 30 recommendations at a time, dedupes by id with `seenIdsRef`, infinite-scrolls via `react-intersection-observer` with a 600px rootMargin. Treats two consecutive empty batches as "no more" — the recommender naturally returns fewer items as cooldowns saturate. Right-click menu includes an "添加到分类" group listing all categories. Multi-select toolbar offers 全选 / 喜欢 / 不感兴趣 / 加入分类 (dislike removes from the list like the single-item path, refilling if it runs short).
- `views/CategoryView.tsx` — same masonry, but loads the full category once via `getCategoryItems` (no pagination — categories are small). Reloads when `categoryRefreshNonce` changes (post-move refresh). Right-click "从分类移除" goes through a `ConfirmDialog`. Remounted via `key={view.id}` on category switch. Multi-select toolbar offers 全选 / 喜欢 / 加入分类 / 从分类移除 (the batch remove also goes through a `ConfirmDialog`).
- `components/MediaCard.tsx` — renders `serendip://thumb/<id>` images, hover-plays videos via `serendip://video/<id>`, calls `setLiked` / `setDisliked`. Registered as a dnd-kit `useDraggable` (`data:{type:'media', item}`); it deliberately does **not** consume the draggable `transform` (the `DragOverlay` is the visual instead) — it only fades the source via `isDragging`. Subscribes to `useSelectionStore` for its own `selected` / `active` state: shows a checkbox + selection ring in selection mode (hiding the hover menu button), and routes clicks to `onSelectClick(item, mods)` when in selection mode or with Ctrl/Cmd/Shift held. **Long-press (500ms) enters selection mode**: implemented by composing the dnd `onPointerDown` listener with a long-press timer that a >8px move (i.e. a real drag) or pointer-up cancels — and a `longPressFired` ref swallows the trailing `click`. The checkbox carries **no z-index** on purpose (a positive z would lift it over the sticky header); the root is `outline-none` so Shift-click doesn't show a focus box.
...
- `components/ContextMenu.tsx` — floating menu **rendered through `createPortal` to `document.body`** (escapes the sidebar's `sticky` stacking context, which otherwise let main-pane cards paint over it). Items support `divider` and `header` (non-clickable group label) besides normal entries. `placement` prop: `'cursor'` (default, anchor top-left at x/y with edge-flip) or `'top'` (open upward, centered on x — used by the toolbar's "加入分类" picker).
- `components/SelectionToolbar.tsx` — floating multi-select bar (`fixed bottom-center`, glass tint). **Rendered whenever selection mode is `active`, even with nothing selected** (count-dependent action buttons disable at 0); only ✕ leaves the mode. First button is a tri-state 全选 toggle (empty box → 全选 / half box → 全选 / full box → 清空, the last calling `onDeselectAll` to stay in mode). Buttons render conditionally on which handlers a view passes, so one component serves both views. The "加入分类" button opens `ContextMenu` with `placement="top"` anchored above the button.
- `components/DragPreview.tsx` — the `DragOverlay` contents: `MediaDragPreview` (thumbnail, play badge for videos, plus a stacked-card shadow + count badge when `count>1` — the multi-drag case) and `CategoryDragPreview` (folder pill).
...
- `media` → dropped on a category row. **Multi-drag:** if the dragged card's id is in the selection set and >1 is selected, the whole set is the payload; the `DragOverlay` shows a count badge. `handleDragStart` / `handleDragEnd` read the selection from `useSelectionStore.getState()` rather than threading it through draggable data. **Move vs copy:** if the drag originated from a `CategoryView` (current `view.kind==='category'`) and the target is a *different* category, App opens a `ContextMenu` (anchored at the target row's right edge via `e.over.rect`) offering 移动到此 (add to target + remove from source, then `bumpCategoryRefresh()`) / 复制到此 (add to target only). Drags from Explore, or onto the same category, just add directly.
...
- Theme colors are CSS variables in `assets/main.css` mapped to Tailwind tokens in `tailwind.config.js`. Frosted overlays (header, multi-select toolbar) use `bg-glass backdrop-blur-xl` — `--color-glass` is a white tint in light mode / black tint in dark mode, layered over the blur for readability. Add a Tailwind color token whenever you introduce a new `--color-*` var, or the class won't generate.
